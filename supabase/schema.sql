...
grant update on public.notifications,public.notification_preferences,public.member_payment_methods to authenticated;
...
-- v3.3.4 balance, rent, and repayment clarity read model.
...
begin
  ...
  select max(bp.month) into v_active_month
  from public.billing_periods bp
  where bp.household_id = c.household_id
    and bp.status = 'active';
