...
create table public.obligations (
  ...
  debtor_member_id uuid not null references public.household_members(id), creditor_member_id uuid references public.household_members(id), creditor_label text,
...
create table public.payments (
  ...
  idempotency_key text not null, verified_by uuid references public.household_members(id), created_at timestamptz not null default now(), unique(household_id,idempotency_key), check(payer_member_id<>payee_member_id)
);
...
create table public.paylater_accounts (
  ...
  original_total_cents bigint not null check(original_total_cents>0), status text not null default 'active' check(status in ('active','completed','void')),
...
create table public.paylater_installments (
  ...
  status text not null default 'scheduled' check(status in ('scheduled','posted','paid','void')), source_expense_id uuid references public.expenses(id), unique(account_id,sequence_no)
);
