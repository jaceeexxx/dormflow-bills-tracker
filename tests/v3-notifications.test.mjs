...
test('push is opt-in and server delivery is separate from financial writes',()=>{const client=fs.readFileSync('js/push.js','utf8');const server=fs.readFileSync('api/push-deliver.js','utf8');assert.match(client,/Notification\.requestPermission/);assert.match(client,/pushManager\.subscribe/);assert.match(server,/web-push/);assert.doesNotMatch(server,/create_expense_v3|review_payment_claim_v3/);});
