...
export async function openPayLaterSheet({identity,existing=null,onDone=()=>{}}){
  const members=await supabase.select('household_members','select=id,profiles(display_name)&is_active=eq.true');
  ...
  bindSaveFlow(form,{idleLabel:'Save',successMessage:'Successfully saved',close:()=>sheet.close(),save:async data=>{
    ...
    if(targetId)await requestPushForTarget({targetType:'paylater',targetId});
    return targetId;
  },onSaved:onDone});
}
