...
export async function openAvatarCropper(file,{onUse,onCancel}={}){
  ...
  const overlay=document.createElement('dialog');overlay.className='avatar-cropper-overlay';overlay.setAttribute('role','dialog');overlay.setAttribute('aria-modal','true');overlay.setAttribute('aria-label','Crop profile photo');
  ...
  return new Promise(resolve=>{
    const finish=value=>{URL.revokeObjectURL(url);if(overlay.open)overlay.close();overlay.remove();resolve(value);};
