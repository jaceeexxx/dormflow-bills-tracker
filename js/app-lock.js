...
export function validateLocalPin(pin){return /^\d{6}$/.test(String(pin));}
...
export async function setLocalPin(userId,pin,storage=localStorage){if(!validateLocalPin(pin))throw new Error('Use a 6-digit PIN.');const salt=crypto.randomUUID();const verifier=await hashPinVerifier(pin,salt);storage.setItem(`dormflow:v3:lock:${userId}`,JSON.stringify({mode:'pin',salt,verifier}));}
