...
function collectAllocations(form){
  return [...form.querySelectorAll('[data-obligation]')].map(input=>({obligation_id:input.dataset.obligation,amount_cents:Math.round(Number(input.value)*100)})).filter(row=>row.obligation_id&&row.amount_cents>0);
}

export async function openClaimReview(claim,{onDone=()=>{}}={}){
  let allocations=claim.suggested_allocations||[];
  if(!allocations.length){const rows=await supabase.rpc('open_obligations_v3',{p_debtor:claim.payer_member_id,p_creditor:claim.payee_member_id});allocations=suggestOldest(rows,claim.amount_cents).allocations;}
