...
export async function loadAdminOverview(){
  ...
  const [expenses,obligations,allocations,memberRows,payments,announcements,periods,paylaterAccounts,paylaterInstallments]=await Promise.all([
...
function settlementRow(row){
  ...
  return `<div class="settlement-row settlement-financial-row"><div class="member-avatar" style="--member-accent:${escapeHtml(row.accent||'#64756f')}">${escapeHtml(row.name.slice(0,1))}</div><div class="settlement-main"><div class="settlement-name"><strong>${escapeHtml(row.name)}</strong><span class="status-pill ${row.status}">${row.status==='settled'?'Settled':'Open'}</span></div><div class="settlement-metrics"><span><small>Needs to pay</small><b>${formatPeso(row.needsToPayCents||0)}</b></span><span><small>Owed to member</small><b>${formatPeso(row.owedToMemberCents||0)}</b></span><span><small>Net position</small><b class="${netClass}">${netText}</b></span></div></div></div>`;
}
