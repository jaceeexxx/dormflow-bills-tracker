# DormFlow v3.3.4 - 20 St. Paul

DormFlow v3.3.4 is the balance, rent, and repayment clarity release. It keeps the v3.3 active-month, Inbox, realtime, v3.3.1 Admin/PayLater, v3.3.2 accounting stabilization, and v3.3.3 financial-write/push repairs while adding the detailed member balance read model, Rent admin action, payment-sheet breakdowns, due-status grouping, and a calmer announcement ticker.

## Upgrade an existing v3.3.3 household

Use this exact order:

1. Back up the current project/database.
2. Overlay the v3.3.4 application patch.
3. In Supabase SQL Editor, run **only** `supabase/migrate-v3.3.4.sql` once.
4. Do **not** rerun `schema.sql`, `seed-members.sql`, `migrate-history.sql`, `migrate-v3.2.sql`, `migrate-v3.3.sql`, `migrate-v3.3.1.sql`, `migrate-v3.3.2.sql`, or `migrate-v3.3.3.sql` on an existing v3.3.3 database.
5. Run `npm test` and `npm run check`.
6. Deploy with `vercel deploy --prod` or push the linked Git repository.
7. Fully close and reopen the installed iPhone PWA once so the v3.3.4 service worker takes control.
...
After deploying v3.3.4 on the installed Home Screen PWA:
...
For a new project use `schema.sql` -> create the four Auth accounts -> `seed-members.sql` -> `migrate-history.sql`. The fresh schema/history files already contain the current v3.3.4 read model and corrected legacy PayLater source values; do not run additive migrations after a fresh setup.
